...
- 初始 consumer inventory 来自 HEAD `0b4b278`。已接受的 Operation Facts Task 3 事实基线为 HEAD `456bb23`：Reducer 32/32（含 6 个真实 normal-import `swiftc -typecheck` client）、CleaningEngine 31/31、OutcomeSideEffectPolicy 21/21、453-test full suite、15 个显式环境 skip、0 failure；Task 3 冻结独立审查为 0 Critical/Important。执行任何 task 前必须重新读取当前 HEAD、`git status --short` 与 `.superpowers/sdd/xico-opfacts-task-3-report.md`；若后续报告增长，以更新后的更高计数为准，绝不回退到较低基线。
...
`CleaningReport.merging(_:supplemental:purpose:id:parentID:occurrenceOrder:)` accepts only the closed `CleaningOperationPurpose`, requires every deletion child `report.operation.kind == purpose.operationKind` and every supplemental child to use its exact registered kind, then validates each child against its payload before flattening it. The final report stores ordered `CleaningItemResult` deletion facts plus typed `CleaningAuxiliaryItemResult` facts (initially `.threatRemediation`) and exactly one new parent outcome; it never stores a child outcome/result. Deletion and auxiliary facts have distinct reducer request IDs, while `relatedCleaningRequestID` provides exact correlation without URL or caller-ID guessing. The explicit parent inventory—not dictionary order or child completion order—defines stable per-occurrence fact order: deletion `D`, then its optional remediation `R`; every remediation relation must resolve to exactly one deletion occurrence.
...
The one deliberate exception is an exact validated Trash receipt (`originalURL` and `trashedURL`) stored only in the dedicated history archive needed for undo. Tests assert its parent directory is `0700`, archive/staging/lock files are `0600`, no backup/recovery artifact is created or remains, schema 1 encodes each URL pair only under its canonical item receipt field (the top-level `restorable` key is temporary schema-0 compatibility), and permanent/non-restorable facts contain no path. `updateRestorable` must reject additions, changed URL pairs and anything other than an exact remove-only subset of the record's existing receipt set: schema 1 requires a validated succeeded-trash+changed item binding, while schema 0 may only retain/clear exact pre-existing legacy pairs and remains untrusted for success. Receipt paths must never appear in normal record metadata, logs, notifications or invalidation events. Do not add an encryption dependency in this phase; encrypted receipt storage may be recorded as future hardening. UI localizes stable issue codes at presentation time. OSLog for touched workflow files uses `.private` or redacted identifiers for path/error/endpoint data.
...
Expected: 0 failures; test count is greater than the latest accepted Operation Facts baseline (453 tests on HEAD `456bb23` at plan revision time, or any higher count recorded later). A lower count is a regression even if it exceeds the old 373/423-test snapshots. Every skip has a written environment reason, none covers an outcome requirement, and the current 15-skip full-suite environment baseline may not silently grow.
...
13. The full suite exceeds the latest accepted Operation Facts baseline (453 at HEAD `456bb23`, or a higher subsequently recorded baseline) and no public Decodable/fact/DTO/legacy scalar escape hatch remains.
